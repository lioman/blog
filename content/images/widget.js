...
//  
  ...
  for (i = 0; i < jtw_num_tweets; i++) {
	...
	ret = ret + '<b><a style="' + jtw_tweet_linkcolor + 'text-decoration:none;" target="_new" href=http://twitter.com/' + user + '>' + user + '</a></b>: ' + tweet + '<br><span class="jtw_tstamp" style="font-size:10px;">' + tstamp; //+ jtw_getdatediff(now_ms, tstamp); //+ tstamp;
	ret = ret + '</span>';
...
if (!jtw_search)
	jtw_search = 'iranElection';
else
	jtw_search = jtw_search;
if (!jtw_num_tweets)
	jtw_num_tweets = 20;
if (!jtw_pre_html)
	jtw_pre_html = 'Tweets zu <b>#iranElection</b>';
if (!jtw_post_html)
	jtw_post_html = '';
if (!jtw_mid_html)
	jtw_mid_html = '';
...
document.write('</div>');
//document.write('<div style="width:100%;background:#000;height:18px;color:#fff;text-align:center;">Widget by TweetGrid</div>');
document.write('</div>');
if (jtw_search != '-') {
